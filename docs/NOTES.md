...
ただし一番上は終わりの秒数の選択欄、最下段は停止ボタンがそれぞれ1行まるごと使うので、
**秒数は間の3行・15個まで**（選択欄を足す前は20個までだった）。
行は自動配置に任せず明示している。任せると停止ボタンが秒数の列に
混ざり、慌てているときに押し間違えるため。

9個なら 1行目が選択欄、2・3行目が 5個+4個、4行目が停止ボタンになる。
