...
| 週 | 貼る場所 | 例 |
|---|---|---|
| 鯖戦 | `PANEL_CHANNEL_IDS` ＋ `PANEL_CHANNELS_SERVER_WAR`（493のサーバー） | 9/12、10/10 |
| 国内戦 | `PANEL_CHANNEL_IDS` ＋ `PANEL_CHANNELS_DOMESTIC`（【JYP】自由の誉火） | 9/26、10/24 |
| 戦闘なし | `PANEL_CHANNEL_IDS` だけ（ノリーのテストサーバー） | 9/19、10/3 |

- 週は `BATTLE_ANCHOR_DATE`（鯖戦だった土曜日）から数えて判定する。鯖戦・国内戦はそれぞれ4週ごとで、2週ずらして交互に来る。
  日曜の深夜に起動しても直前の土曜日で判定する。起動ログに「今週（10/10）は鯖戦」のように出る
- **毎週貼るのはテストサーバーだけ。** ゲーム用のサーバーには戦闘がある週だけ貼る（毎週だと通知がうるさいため）
