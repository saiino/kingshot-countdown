...
`.env` の `PANEL_CHANNEL_IDS` にチャンネルIDを書くと、起動するたびにそこへパネルを貼る。

- **今はテストサーバーだけ。** ゲーム用のサーバーに毎回貼ると通知がうるさいため
- ボイスチャンネルのチャット欄に貼るなら、ボイスチャンネルのIDをそのまま書く
- 前回の起動で貼ったものは消してから貼るので、溜まらない。どれを貼ったかは `announced_panels.json`（`.gitignore` 済み）に残る
