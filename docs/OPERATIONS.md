...
## 試作中: `/panel_dev`（終わりの秒数を選べるパネル）

ボタンを押していたメンバーからの要望で試している。
...
- 今の `/panel` とはボタンのIDが別（`kingshot:dev:`）なので、並べて貼っても干渉しない
- 同じサーバーに試作パネルを2枚貼ると、変えなかったほうの文言は古いまま。
  **流れるのは押した瞬間の保存値**なので、動きは正しい

本採用するときは `/panel` 側に取り込む。IDが変わるので、そのとき試作パネルは貼り直しになる。
