...
起動時に貼ったパネルを「出陣完了」に書き換え、ログに「終了しました」を残してから終わる。
...
- **止めるときは Ctrl+C のほうがよい。** 「終了しました」が記録され、起動時に貼ったパネルも「出陣完了」になる。
...
- **鯖戦の夜は、JYPにパネルの代わりに挨拶だけ出す**（「☀️ 出陣ベル、待機中です！いつでも呼んでね📯 使いたかったら /panel 打ってね〜」）。
...
- その夜に使わないチャンネル（戦闘の夜以外の493やJYP）のメッセージは、「出陣完了」のまま触らない。止まるときに書き換えるのも、その夜に使ったチャンネルだけ
...
- **メッセージは増やさない。** チャンネルごとに1通だけを使い回し、パネル・挨拶・出陣完了と中身を書き換えていく。手で消されていたときだけ新しく送る。どのメッセージを使っているかは `announced_panels.json`（`.gitignore` 済み）に残る
- 止まるときは、そのパネルを「💤 出陣完了！お疲れさまでした。またね〜👋」に書き換えてボタンを外す。停止中に押されて「応答しませんでした」になるのを防ぐ
- 書き換えが走るのは **Ctrl+C** と **`stop_bot.ps1`（タスクスケジューラ）** で止めたとき。
  ×で閉じる・PCのスリープ・落ちたときは走らず、ボタン付きのまま残る（次の起動で書き換わる）
- Botが自分から投稿するので、そのチャンネルでBotに「チャンネルを見る」「メッセージを送信」の権限が要る。
  無いと起動時のログに「パネルを貼れませんでした」と出る
- `/panel` を打ったチャンネルにBotの1通（挨拶や出陣完了）があれば、新しく出さずにそれをパネルに書き換え、打った人にだけそのメッセージへのリンクを返す。無ければ新しく出して、以後そのチャンネルの1通として使い回す（止まるときに出陣完了になる）。これで `/panel` を何度打ってもチャンネルのメッセージは増えない
...
- Botを動かしたままでも使える。ただし「前回貼ったパネル」の記録（`announced_panels.json`）はBotと共有なので、Botが起動時に出したメッセージもこれで書き換わる
